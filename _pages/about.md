---
layout: home
title: about
permalink: /
subtitle: Economist · cognitive economics · network theory
home_variant: a

home:
  kicker: >
    Economist<span class="kicker-sep"> · </span><br class="kicker-break">Cognitive Economics · Network Theory
  headline: How people reason.<br>How groups coordinate.
  lede: >
    I study how people reason and how groups coordinate. My research is focused on
    <strong>cognition</strong>, where I work on modeling the human reasoning process, and <strong>networks</strong>, where I study collective
    action, contagion, and influence.
  profile_caption: >
    Postdoctoral Fellow, <a href="https://www.cognitiveeconomics.com/" rel="noopener">Manos Institute for Cognitive Economics</a> / UNSW Sydney
  actions:
    - label: Start here
      url: /start/
    - label: Research
      url: /projects/
    - label: Writing
      url: /blog/
    - label: Tools
      url: /tools/
  selected_work_heading: Selected Work
  selected_work_all_label: All research
  selected_work_all_url: /projects/
  selected_work:
    - title: Bad Networks
      status: Journal of Public Economics, 2026
      url: /assets/pdf/Bad_Networks_JPubE.pdf
    - title: Targeted Persuasion
      status: "American Economic Journal: Microeconomics (conditionally accepted)"
      url: /assets/pdf/Targeted_Persuasion_Aug_2026.pdf
    - title: Strategic Influence
      status: R&R, Econometrica (2nd round)
      url: /assets/pdf/Strategic_Influence_July_2026.pdf
    - title: Controlling Complex Contagions
      status: Working paper
      url: /assets/pdf/Controlling_Complex_Contagions_Sep_2026.pdf
    - title: Ghostwriter for Anki
      status: Research tool
      url: /ghostwriter-for-anki/
  agenda_kicker: Research Agenda
  agenda:
    - title: Cognition
      text: >
        How can we model the human reasoning process? My new agenda at the <a href="https://www.cognitiveeconomics.com/" rel="noopener">Manos Institute</a> is to develop formal models of cognition. Our first papers will be coming in 2026–27.
    - title: Networks
      text: >
        How does interaction shape collective action? I study contagion, influence, and network design: when social structure helps groups
        coordinate, and when it traps them in bad equilibria.
  questions_kicker: Open Questions
  questions_headline: Writing on economics, AI, and tools&nbsp;for&nbsp;thought.
  questions_text: Subscribe for my non-academic writing.

profile:
  align: right
  image: dj.png
  image_circular: false
  more_info: >
    <p>Sydney, Australia</p>

news: true
selected_papers: true
social: true
---
